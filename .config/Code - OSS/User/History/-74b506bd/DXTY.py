...
# CNAPS
def cnaps(salaire):
    cnaps = int((salaire * 1)/100)   # Retenu par l'employeur
    return cnaps


# CDS
def cds(salaire):
    cds = int((salaire * 2)/100)   # Retenu par l'employeur
    return cds


# OSTIE
def ostie(salaire):
    ostie = int((salaire * 1)/100)   # Retenu par l'employeur
    return ostie


# SI
def si(salaire, cnaps=0, cds=0, osite=0):
    si = salaire - cnaps - cds - ostie
    return si
...
if __name__=='__main__':
    salaire = 222_300
    si = si(salaire, cnaps(salaire), cds(salaire), ostie(salaire))
    print(f"Salaire: {salaire}")
    print(f"SI: {si}")
    print(f"IRSA: {irsa(si)}")
    print(f"Salaire Net: {si-irsa(si)}")