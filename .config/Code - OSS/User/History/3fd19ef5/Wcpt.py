...
def add_patient():
    ...
    curs = con.cursor()
    curs.execute("INSERT INTO patient('id', 'firstname', 'lastname', 'age', 'address', 'phone', 'remark') VALUES(?,?,?,?,?,?,?)")
    con.commit()
    con.close()
...
def edit_patient():
    ...
    curs = con.cursor()
    curs.execute(
        "UPDATE patient SET firstname=?, lastname=?, age=?, address=?, phone=?, remark=? WHERE id=?",
...
# Title
lbl_title = Label(root, text="GESTION DES PATIENTS CHE THopital", font=('Arial', 30), bg='#81A1C1', fg='#D8DEE9')
...
con = sqlite3.connect('hospital.db')
curs = con.cursor()
select = curs.execute("SELECT * FROM patient")
for row in select:
