...
def register_student():
    ...
    try:
        sql = "INSERT INTO student (id, firstname, lastname, gender, level, ue, note) VALUES (%s, %s, %s, %s, %s, %s, %s)"
        val = (id_s, firstname, lastname, gender, level, ue, note)
        curs.execute(sql, val)
        con.commit()
        last_id_s = curs.lastrowid
        messagebox.showinfo('Info', 'Etudiant ajouter')
        root.destroy()
        call(['python', 'main.py'])
...
def edit_student():
    ...
    try:
        sql = "UPDATE student SET firstname=%s, lastname=%s, gender=%s, level=%s, ue=%s, note=%s, WHERE id=%s"
        val = (firstname, lastname, gender, level, ue, note, id_s)
        curs.execute(sql, val)
        con.commit()
        last_id_s = curs.lastrowid
        messagebox.showinfo('Info', 'Etudiant modifier')
        root.destroy()
        call(['python', 'main.py'])
    except Exception as e:
        print(e)
...
def delete_student():
    ...
    try:
        ...
        con.commit()
        last_id_s = curs.lastrowid
        messagebox.showinfo('Info', 'Etudiant supprimer')
